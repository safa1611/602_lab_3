node('linux') {
        ...
        stage('Create Test Stack') {
             ...
             withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: '633ca12c-f866-45d4-9173-1fd27644f88b', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
                       sh 'aws cloudformation create-stack --stack-name test2 --region us-east-1 --template-body file://s3.amazonaws.com/safa1611-assignment-4/docker-single-server.json --parameters ParameterKey=KeyName,ParameterValue=safa ParameterKey=YourIp,ParameterValue=$(curl ifconfig.me)/32'
