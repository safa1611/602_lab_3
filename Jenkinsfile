node('linux') {
        ...
        stage('Create Test Stack') {
             ...
             withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: '0776525b-8d0c-4b04-9237-4625bfc903d3', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
                   ...
                   sh 'aws cloudformation create-stack --stack-name test2 --region us-east-1 --template-url https://s3.amazonaws.com/safa1611-assignment-4/docker-single-server.json --parameters ParameterKey=KeyName,ParameterValue=safa1611 ParameterKey=YourIp,ParameterValue=$(curl ifconfig.me)/32'
